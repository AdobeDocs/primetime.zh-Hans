---
title: 具有动态客户端注册的Android SDK
description: 具有动态客户端注册的Android SDK
...
# 具有动态客户端注册的Android SDK {#android-sdk-with-dynamic-client-registration}
...
修改了适用于Android的Android AccessEnabler SDK，以在不使用会话Cookie的情况下启用身份验证。 随着越来越多的浏览器限制对Cookie的访问，需要使用另一种方法来允许身份验证。

对于Android，使用Chrome自定义选项卡会限制从其他应用程序访问Cookie的权限。

>**Android SDK 3.0.0** 介绍：

- 动态客户端注册根据签名的请求者ID和会话cookie身份验证替换当前应用程序注册机制
- 用于身份验证流的Chrome自定义选项卡

>[!NOTE]
>
>对于不支持Chrome自定义选项卡的旧版Android版本，将使用与旧版AccessEnabler SDK类似的WebView身份验证。


## 动态客户端注册 {#DCR}

Android SDK v3.0+将使用在 [动态客户端注册](/help/authentication/dynamic-client-registration.md).


## 功能演示 {#Demo}

请看 [此网络研讨会](https://my.adobeconnect.com/pzkp8ujrigg1/) 该功能提供了功能的更多上下文，并包含了如何使用TVE Dashboard管理软件语句的演示，以及如何使用Adobe作为Android SDK的一部分提供的演示应用程序来测试生成的语句。
...
- redirectUrl :唯一url，即TVE功能板或 *null* 如果在strings.xml中设置“redirect\_uri”

注意：无效的softwareStatement或redirectUrl将导致应用程序无法初始化AccessEnabler或注册应用程序以进行Adobe Pass身份验证和授权
</br>
注意：strings.xml中的redirectUrl参数或redirect\_uri应该是在TVE功能板中为应用程序添加的域的值，其顺序应当为反向(例如：对于TVE Dashboard中添加的域“adobe.com”，redirectUrl应为“com.adobe”。
 
...
- *requestorID*:与渠道关联的唯一ID。 在您首次使用Adobe Primetime身份验证服务进行注册时，将由Adobe分配的唯一ID传递到您的网站。
- *url*:可选参数；默认情况下，使用Adobe服务提供程序 [http://sp.auth.adobe.com/](http://sp.auth.adobe.com/). 此数组允许您为由Adobe提供的身份验证和授权服务指定端点（可能出于调试目的而使用不同的实例）。 您可以使用它指定多个Adobe Primetime身份验证服务提供程序实例。 这样做时，MVPD列表由所有服务提供商的端点组成。 每个MVPD都与最快的服务提供商关联；即首先响应并支持该MVPD的提供商。
...
a.从Adobe Pass获取software\_statement和redirect\_uri(TVE Dashboard)

b.有两个选项可将这些值传递到Adobe Pass SDK:
...
| HTTP 400（请求错误） | {&quot;error&quot;:&quot;无效\_client&quot;} | 客户端身份验证失败，因为客户端未知。 SDK必须再次向授权服务器注册。 |
| HTTP 400（请求错误） | {&quot;error&quot;:&quot;unauthed\_client&quot;} | 已验证的客户端无权使用此授权授权类型。 |

- 如果MVPD需要被动身份验证，则将打开Chrome自定义选项卡以使用该MVPD执行被动身份验证，并在完成后关闭
...
- mvpd记住：转到setSelectedProvider(mvpd_id)
...
- 已取消登录：重置MVPD选择
- URL方案建立为“adobepass://redirect_uri”，用于在身份验证完成时捕获
...
- SDK将打开Chrome自定义选项卡以访问mvpd_id注销端点。 完成后，将关闭Chrome自定义选项卡
- URL方案建立为“adobepass://logout”以捕获注销完成时的时刻
- logout将触发sendTrackingData(new Event(EVENT_LOGOUT，USER_NOT_AUTHENTICATED_ERROR)和回调：setAuthenticationStatus(0,&quot;Logout&quot;)
