...
“代理MVPD”是一个MVPD，它除了管理自身与Adobe Primetime身份验证的集成之外，还代表一组关联的“代理MVPD”管理授权进程。 这种安排对程序员是透明的。
...
Adobe Primetime身份验证建议ProxyMVPDs仅在上一次推送发生更改时推送其ProxiedMVPDs列表。
...
* `ProviderID` （可选） - ID元素上存在ProviderID属性时，ProviderID的值将在SAML身份验证请求中作为代理的MVPD/SubMVPD ID发送到代理MVPD（而不是ID值）。 在这种情况下，ID的值将仅用在程序员页面上展示的MVPD选取器中，并在内部由Adobe Primetime身份验证使用。 ProviderID属性的长度必须为1至128个字符。
