...
适用于Amazon FireOS的Adobe Primetime身份验证权利解决方案最终分为两个域：
...
1. 启动Adobe Primetime身份验证
   ...
   2. 调用` setRequestor()` 建立程序员的身份，传递程序员的身份 `requestorID` 和（可选）Adobe Primetime身份验证端点数组。
...
1. 用户选择提供程序后，WebView将打开供用户登录的提供程序页面
   ...
   **注意：** 此时，用户可以取消身份验证流程。 如果发生这种情况，AccessEnabler将清理其内部状态并重置身份验证流程。
