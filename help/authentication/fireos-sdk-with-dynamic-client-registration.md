...
- *softwarestatement*：从TVE仪表板或获取的值 *null* 如果在strings.xml中设置了“software\_statement”
- *redirectUrl* ：对于FireTV实施，此参数应为空。 将忽略此属性上的任何设置。 
...
如果 *url* 未使用参数，生成的网络调用将定位默认服务提供商URL：Adobe发布生产环境。
...
- c. getAuthentication ：SDK将包含 **access_token** 在调用参数中 
   ...
   - mvpd已记忆：转到setSelectedProvider(mvpd\_id)
