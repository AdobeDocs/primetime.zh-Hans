...
+ Primetime身份验证概念 {#authentication-concepts}
   ...
   + [程序员概述](programmer-overview.md)
   + [MVPD概述](mvpd-overview.md)
+ 引题入门指南 {#kickstart-guides}
   + [程序员启动指南](programmer-kickstart-guide.md)
   + [MVPD启动指南](mvpd-kickstart-guide.md)
+ 程序员集成指南 {#programmer-integration-guide}
   + [程序员集成指南概述](programmer-integration-guide-overview.md)
   + [程序员授权流程](entitlement-flow.md)
   ...
   + REST API {#restapi}
      ...
      + Rest API引用 {#rest-api-reference}
         + [REST API引用](rest-api-reference.md)
         + [注册代码请求](registration-code-request.md)
         ...
         + [检索预授权资源列表](retrieve-list-of-preauthorized-resources.md)
         + [通过第二屏幕Web应用程序检索预授权资源列表](retrieve-list-of-preauthorized-resources-by-second-screen-web-app.md)
         + [启动注销](initiate-logout.md)
         + [用户元数据](user-metadata.md)
         + [检索profile-request](retrieve-profilerequest.md)
   ...
   + AccessEnabler SDK {#accessenabler-sdk}
      ...
      + Android SDK {#androidsdk}
         ...
         + 准则 {#androidguidelines}
            ...
            + [具有动态客户端注册的Android SDK](android-sdk-with-dynamic-client-registration.md)
         + Android API{#androidapi}
            + [预授权](preauthorize-android.md)
      + Amazon FireOS SDK {#fireossdk}
         + [Amazon FireOS SSO — 程序员启动指南](amazon-firetv-sso-programmer-kickoff-guide.md)
         + [Amazon FireOS单点登录（使用无客户端API指南）](amazon-fireos-sso-using-clientless-api-cookbook.md)
         ...
         + [具有动态客户端注册的FireOS SDK](fireos-sdk-with-dynamic-client-registration.md)
   + 平台SSO {#platform-sso}
      + Apple SSO {#apple-sso}
   ...
   + 内容元数据 {#content-metadata}
      + [识别受保护的资源](identify-protected-resources.md)
...
+ Primetime身份验证功能 {#auth-features}
   ...
   + 临时通道 {#temp-pass}
      + [临时通道](temp-pass.md)
      + [提升临时通行证](promotional-temp-pass.md)
   + 单点登录 {#sso}
      + [单点登录支持](sso-support.md)
      + [通过被动身份验证的单点登录](sso-passive-authn.md)
   + 基于家庭的身份验证 {#home-based-auth}
      + [适用于随时随地电视的基于家庭的身份验证](home-based-authn-tve.md)
   ...
   + 隐私就绪 {#privacy-readiness}
      ...
      + [如何发出隐私请求](make-privacy-req.md)
+ 提示和疑难解答 {#tips-troubleshoot}
   + [在选择对话框中允许MVPD](allow-mvpd-selectn-dialog.md)
   + [阻止MVPD显示选择对话框](prevent-mvpd-selectn-dialog.md)
+ 支持 {#support}
   + [呈报程序](escalation-procedures.md)
   + [监控PrimetimeAdobePayTV通行证](monitoring-adobe-pay-tv-pass.md)
...
+ 技术说明 {#tech-notes}
   + Primetime身份验证SDK {#primetime-authentication-sdks}
      + [证书问答](certificates-qa.md)
      + JavaScript SDK {#javascript}
         + [Safari浏览器的JS SDK限制](js-sdk-limitations-for-safari-browser.md)
         + [Cookie更新 — SameSite和安全标记](cookies-updates--samesite-and-secure-flags.md)
      + Android SDK {#android}
         + [在Android 10应用程序上访问启用程序Android SDK单点登录(SSO)](access-enabler-android-sdk-single-signon-sso-on-android-10-devices.md)
         + [Adobe Primetime身份验证和Android 6“棉花糖”新权限模型](adobe-primetime-authentication-and-the-android-6-marshmallow-new-permissions-model.md)
      + iOS/tvOS SDK {#iostvos}
         ...
         + [使用Primetime身份验证访问启用程序时在iOS上使用SSO](sso-on-ios-when-using-the-primetime-authentication-access-enabler.md)
         + [iOS身份验证错误 — 找不到adobepass.ios.app](ios-authentication-error-adobepassiosapp-cannot-be-found.md)
         + [在iOS上重置Temp Pass](reset-temp-pass-on-ios.md)
   ...
   + Primetime身份验证环境 {#primetime-authentication-environments}
      ...
      + [在每季度前设置环境和测试](setting-up-your-environment-and-testing-in-prequal.md)
      + [如何使用AdobeAPI测试网站测试身份验证和授权流程](test-authn-authz-flows-using-adobes-api-test-site.md)
   + 无客户端API {#clientless-api}
      + [无客户端API实施 — 错误代码/具有可能原因/原因的消息](clientless-api-implementation-error-codes--messages-with-probable-reason--cause.md)
      + [没有设备ID时的无客户端API流](clientless-api-flow-in-the-absence-of-device-id.md)
      + [无客户端：避免在/authenticate请求中使用&#39;&amp;&#39;reg_code](clientless-avoid-using-reg-code-in-authenticate-request.md)
      + [在Xbox 360和XboxOne无客户端上为程序启用Primetime授权服务](enabling-primetime-entitlement-services-for-a-programmer-on-xbox-360-and-xboxone-clientless-solution.md)
