...
要在程序员应用程序中使用增强型错误代码，需要向支持团队提出请求，以便通过配置更改来启用它。

## 响应错误处理 {#response-error-handling}

在大多数情况下，Primetime身份验证API在响应正文中包含其他错误信息，以便提供 **有意义的上下文** 发生特定错误的原因和/或自动修复问题的可能解决方案。  *但是，在涉及身份验证或注销流的某些特定情况下，Primetime身份验证服务可能会返回HTML响应或空正文 — 有关详细信息，请查看API文档。*
...
- 4xx错误代码暗示该错误是由客户端生成的，客户端需要执行额外的操作来修复它（例如，在调用受保护的API或提供任何必需的参数之前获取访问令牌）

- 5xx错误代码表示该错误是由服务器生成的，服务器需要执行其他操作来修复它。
...
处理多个项目的Adobe Primetime API（预授权API等）可能会指示处理特定项目是否失败，并使用项目级别错误信息指示处理其他项目是否成功。 在本例中， ***&quot;error&quot;*** 对象位于项目级别，并且响应正文可能包含多个 ***&quot;errors&quot;*** 对象 — 请参阅API文档。
...
| 名称 | 类型 | 示例 | 受限 | 描述 |
|----|----|----|----|--------------|
| 状态 | *整数* | 403 | ♦ | RFC 7231中记录的响应HTTP状态代码(https://tools.ietf.org/html/rfc7231#section-6) <br> - 400个错误请求 <br> - 400个错误请求 <br> - 400个错误请求 <br> - 401未授权 <br> - 403禁止访问 <br> - 404未找到 <br> - 405不允许使用该方法 <br> - 409冲突 <br> - 410已删除 <br> - 412先决条件失败 <br> - 429请求太多 <br> - 500间隔服务器错误 <br> - 503服务不可用 |
| 代码 | *字符串* | network_connection_fail | ♦ | 标准Primetime身份验证错误代码。 下面包含完整的错误代码列表。 |
| 消息 | *字符串* | 无法联系您的电视提供商服务 |  | 可显示给最终用户的可读消息。 |
| 详细信息 | *字符串* | 您的订阅包不包含“实时”渠道 |  | 在某些情况下，MVPD授权端点或程序员通过降级规则提供了详细消息。 <br> <br> 请注意，如果未从合作伙伴服务收到自定义消息，则错误字段中可能不存在此字段。 |
| helpUrl | *url* | &quot;&quot; |  | 一个URL，链接到有关此错误发生原因和可能解决方案的详细信息。 <br> <br>  URI表示绝对URL，不应从错误代码推断。 根据错误上下文，可以提供不同的URL。 例如，相同的bad_request错误代码将为身份验证和授权服务生成不同的url。 |
| trace | *字符串* | 12f6fef9-d2e0-422b-a9d7-60d799abe353 |  | 此响应的唯一标识符，在联系支持人员以识别更复杂场景中的特定问题时，可以使用此标识符。 |
| 操作 | *字符串* | 重试 | ♦ | *建议采取的纠正措施：* </br><br> -none — 很遗憾，没有预定义操作可修复此问题。 这可能表明对公共API的调用不正确 </br><br>-configuration — 需要通过TVE仪表板或联系支持部门更改配置。 </br><br>-application-registration — 应用程序必须重新注册自身。 </br><br>-authentication — 用户必须进行身份验证或重新进行身份验证。 </br><br>-authorization — 用户必须获得特定资源的授权。 </br><br>-degradation — 应该应用某种形式的降级。 </br><br>-retry — 重试请求可能解决此问题</br><br>-retry-after — 在指定的时间段后重试请求可能会解决此问题。 |

</br>

**注释：**

- ***受限*** 列 *指示相应的字段值是否表示有限集* (例如，“”的现有HTTP状态代码&#x200B;*状态*“ ”字段)。 以后对该规范的更新可能会向受限列表添加值，但不会删除或更改现有值。 不受限字段通常可以包含任何数据，但为确保合理的大小，可能存在限制。

- 每个Adobe响应将包含一个“Adobe请求ID”，用于标识整个HTTP服务中的客户端请求。 “**trace**”字段是对的补充，并且它们应一起报告。 

## HTTP状态代码和错误代码 {#http-status-codes-and-error-codes}

由于与旧版sdk和应用程序（例如，向后兼容要求）存在各种错误代码与其关联的HTTP状态代码之间的不一致问题 *未知\_application* 生成400个错误请求，同时 *未知\_software\_statement* 产生401（未授权）。 解决这些不一致问题将成为未来迭代的目标。 
 
## 操作和错误代码 {#actions-and-error-codes}

对于大多数错误代码，多个操作可以作为修复手头问题的途径，甚至可能需要多个操作才能自动修复它们。 我们选择指示修复错误概率最高的那一个。 此 **操作** 可以分为三类：

1. 尝试修复请求上下文（重试、重试之后）的项 
1. 尝试修复应用程序中的用户上下文（应用程序注册、身份验证、授权）的用户 
1. 尝试修复应用程序和身份提供者之间的集成上下文（配置、降级）的用户

对于第一个类别（重试和在此之后重试），仅重试同一请求可能足以解决此问题。 如果API处理多个项目，则应用程序应重复该请求，并仅包含那些具有“重试”或“在此之后重试”操作的项目。 对于&quot;*重试之后*“操作，一个”<u>重试之后</u>”标头将指示应用程序在重复请求之前应等待的秒数。

对于第2类和第3类，实际操作实施在很大程度上取决于应用程序功能。 例如，“*降解*“可以实施为”切换到15分钟临时播放以允许用户播放内容“或”自动工具以应用AUTHN-ALL或AUTHZ-ALL降级以便与指定的MVPD集成“。 类似于&quot;*身份验证*”操作可以在平板电脑上触发被动身份验证（后通道身份验证），并在连接的电视上触发完整的第二屏幕身份验证流程。 因此，我们确实选择提供包含架构和所有参数的完全成熟的URL。 
...
| 操作 | 错误代码 | HTTP状态代码 | 描述 |
|---|---|---|--------------|
| 配置 | *authorization_denied_by_mvpd* | 403 | 在请求对指定资源的授权时，MVPD返回了“拒绝”决定。 |
...
| 重试之后 | *too_many_requests* | 429 | 在指定时间间隔内发送的请求过多。 应用程序可以在建议的时间段后重试该请求。 |
