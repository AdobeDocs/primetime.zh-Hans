...
| --- | --- | --- | --- | --- | --- |
| &lt;sp_fqdn>/api/v1/tokens/authn</br></br>例如：</br></br>&lt;sp_fqdn>/api/v1/tokens/authn | 流应用程序</br></br>或</br></br>程序员服务 | 1.申请人（必填）</br>2.  deviceId（必需）</br>3.  device_info/X-Device-Info（必需）</br>4.  _设备类型_ （已弃用）</br>5.  _设备用户_ （已弃用）</br>6.  _appId_ （已弃用） | GET | 包含身份验证信息或错误详细信息（如果失败）的XML或JSON。 | 200 — 成功。  </br>404 — 未找到令牌  </br>410 — 令牌已过期 |
...
| _设备类型_ | 设备类型（例如，Roku、PC）。</br></br>**注释**：device_info将替换此参数。 |
