...
# 实施DRM回调{#implementing-drm-callbacks}
...
您可以定义回调函数来解析许可证消息和许可证响应，并在对的调用中传递它们 `drmManager.acquireLicense`. `onLicenseResponseCallback` 是中的新参数 `acquireLicense` API。
...
在保护数据方面，新的 **[!UICONTROL certificateResponseType]** 字段用于设置证书响应类型。 以下是保护数据的示例：
...
使用 `certificateResponseType` 字段是可选的。 如果未使用，则假定值为 `ArrayBuffer`.
