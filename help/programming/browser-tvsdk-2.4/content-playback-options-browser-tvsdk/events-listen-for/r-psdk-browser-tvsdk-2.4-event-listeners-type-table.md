---
description: 向浏览器TVSDK注册事件监听器时，您指定要监听的事件类型和回调的名称。 当发生事件时，浏览器TVSDK将调用您的回调并将相应类型的事件对象传递给它。
seo-description: 向浏览器TVSDK注册事件监听器时，您指定要监听的事件类型和回调的名称。 当发生事件时，浏览器TVSDK将调用您的回调并将相应类型的事件对象传递给它。
seo-title: 回调的事件类型和类
title: 回调的事件类型和类
...
向浏览器TVSDK注册事件监听器时，您指定要监听的事件类型和回调的名称。 当发生事件时，浏览器TVSDK将调用您的回调并将相应类型的事件对象传递给它。

<table frame="all" colsep="1" rowsep="1" id="table_FE58AD65AF3B4483816C00D7EAD2FB4F"> 
 <thead> 
  <tr rowsep="1"> 
   <th colname="2" class="entry"> 当您侦听此事件名称时(AdobePSDK.EventType) </th> 
   <th class="entry">phrases/browser-tvsdk-name将事件传递给此对象类型的回调(<span class="codeph"> AdobePSDK.Event</span>) </th> 
  </tr> 
 </thead>
 <tbody> 
  <tr rowsep="1"> 
