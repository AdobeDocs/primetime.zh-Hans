---
description: UI框架是位于浏览器TVSDK之上的UI层，它提供各种现成的与视频播放器相关的UI结构。 您可以通过做出适合您环境的点更改来创建高度可自定义的播放器。
title: UI框架
...
UI框架是位于浏览器TVSDK之上的UI层，它提供各种现成的与视频播放器相关的UI结构。 您可以通过做出适合您环境的点更改来创建高度可自定义的播放器。
...
为了生成JSON对象，浏览器TVSDK提供了UI配置器工具。 在该工具中，您可以选择各种设置，然后单击 **[!UICONTROL Test Configuration]** 以验证设置，然后单击 **[!UICONTROL Download Configuration]** 以下载设置。 下载文件的内容将用作JSON对象，以传递给 `ptp.videoPlayer` API。

**如何运行UI配置器工具**：

1. 托管 `frameworks` 本地Web服务器上的浏览器TVSDK中提供的文件夹。
1. 要打开该工具，请打开浏览器并导航至 `< path-to-hosted-frameworks-folder>/ui-framework/ui-configurator/`.
...
* **将配置对象传递到videoPlayer函数** 使用此对象时，除了上面讨论的播放设置之外，还可以配置UI的行为。 调用方需要指定必须更改的参数，播放器将继续使用未指定参数的默认值。
  ...
  在上述示例中，ABR控制参数是使用配置对象配置的。 还传递了一个对象以配置控件栏行为。

  有关配置对象的结构，请参阅下面的查看配置对象结构部分。

* **访问AdobePSDK.MediaPlayer** 您可以使用 `videoPlayer.getMediaPlayer` 在某些需要访问浏览器TVSDK的MediaPlayer的高级用例中。

* **配置播放器的外观** 有关为播放器蒙皮的更多信息，请参阅 [为播放器蒙皮](../../browser-tvsdk-2.4/c-psdk-browser-2.4-userinterface/c-psdk-browser-tvsdk-2.4-skin-the-player.md).

## 修改默认行为 {#section_D5D692638FFF4BEF81F7BE70E438CCE9}

在UI框架术语中，行为是定义特定组件的可视部分和交互部分的结构。 通过使用下面概述的对象结构，您可以修改要在行为中进行更改的内容。
...
>[!NOTE]
>
>根据所需的自定义设置，您可以在行为中覆盖某些功能或编写您自己的行为。 有关可覆盖哪些功能的更多信息，请参见 [UI框架](https://help.adobe.com/en_US/primetime/api/psdk/btvsdk-ui-framework/index.html) api文档。

## 引用 {#section_0A76A3F44D8A49B09FE4C83F3FACCB76}

以下是一些其他参考信息：

* **查看配置对象结构** 这是一个完整的对象结构，它用行为的默认元素以分层方式提及所有默认行为。 在示例配置中，使用UI工厂创建元素。 您可以使用相同的元素或使用首选方法来构建元素。
...
* **辅助结构** 此结构由以下内容组成：
   ...
   * **Mixins** Mixin是可组合的模块，可以在行为中组合以使用通用结构。 例如，许多组件希望了解可能影响其行为的更改，例如在播放广告时。 所有这些元素将添加 `adBreak` 类。
   ...
   * **实用工具** 某些实用程序，例如 `ptp.elementGetter`，在配置部分中使用和 `ptp.deepmerge`，可以帮助您编写或扩展行为。 欲了解更多信息，请参见 [UI框架](https://help.adobe.com/en_US/primetime/api/psdk/btvsdk-ui-framework/index.html) api文档。
