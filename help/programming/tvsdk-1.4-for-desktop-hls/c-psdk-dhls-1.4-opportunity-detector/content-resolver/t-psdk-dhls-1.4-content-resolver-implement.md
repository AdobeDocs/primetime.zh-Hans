...
当TVSDK检测到新机会时，它会循环访问注册内容解析器，寻找能够使用该方法解决该机会的内 `canResolve` 容解析器。 选择返回true的第一个用于解决该机会。 如果没有内容解析程序功能，则跳过该机会。 由于内容解析过程通常是异步的，因此内容解析程序负责在进程完成时通知TVSDK。

* 内容解析程序调 `client.place` 用以指定TVSDK需要执行哪些时间轴操作（通常是广告中断放置）。
* 如果解析过程成 `client.notifyCompleted` 功，或进程失败，则内容解析 `client.notifyFailed` 程序将调用。
