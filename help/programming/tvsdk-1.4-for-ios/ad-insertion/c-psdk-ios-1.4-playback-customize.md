...
>[!TIP]
>
>可以使用覆盖默认行为 `PTAdPolicySelector` 类。

默认行为会有所不同，具体取决于用户是在正常播放期间还是通过在视频中进行搜寻而经过广告时间。
...
## 广告播放的API元素 {#section_296ADE00CFEA40CBA1B46142720D13A5}
...
<table id="table_B07E373B9D2B425AB36466B1D42411AD"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> 控制是否应将广告时间标记为已被观看者观看，如果是，何时将其标记为已观看。 使用设置和获取受监视的策略 <span class="codeph"> adBreakAsWatched </span> 属性。 </td> 
  </tr> 
  <tr> 
...
在自定义或覆盖广告行为之前，请先使用TVSDK注册广告策略实例。

要自定义广告行为，请执行以下操作之一：

* 遵守 `PTAdPolicySelector` 协议和实现所有需要的策略选择方法。
...
1. 通过客户端工厂注册TVSDK要使用的策略实例。
   ...
   >[!NOTE]
   >
   >在播放开始时注册的自定义广告策略在以下情况下清除： `PTMediaPlayer` 实例已取消分配。 每次创建新播放会话时，您的应用程序都必须注册一个策略选择器实例。
...
默认情况下，当用户搜索广告时间时，TVSDK会强制播放广告时间。 如果距上一个广告时间完成的时间在特定的分钟数内，您可以自定义跳过广告时间的行为。

>[!IMPORTANT]
>
>当存在跳过广告的内部搜寻时，播放中可能会稍微暂停。

以下自定义广告策略选择器的示例在用户观看广告时间后的五分钟（墙上时钟时间）内跳过广告。
...
## 保存视频位置并在稍后恢复 {#section_FAE252E38CED48D4BDD38BAA4A6A20A4}
...
1. 当用户退出视频时，您的应用程序将检索并保存视频中的位置。
   ...
   例如，如果用户位于视频的第20分钟，并且此位置包含五分钟的广告， `currentTime` 1200秒，而 `localTime` 在此位置将为900秒。
...
1. 要在同一位置恢复视频：要从上一个会话中保存的位置恢复播放视频，请使用 `seekToLocalTime`
