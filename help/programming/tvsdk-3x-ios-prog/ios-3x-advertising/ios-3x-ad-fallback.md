...
1. 将`FallbackOnInvalidCreativeEnabled`设置为`YES`，当线性／内联广告的媒体类型对HLS无效时，VMAP将回退。

   >[!NOTE]
   >
   >默认值为NO。 如果线性广告失败，因为它的媒体类型无效，或者广告无法重新打包，则此标志允许Primetime广告决策遵循与广告是空VAST包装器相同的回退行为。
