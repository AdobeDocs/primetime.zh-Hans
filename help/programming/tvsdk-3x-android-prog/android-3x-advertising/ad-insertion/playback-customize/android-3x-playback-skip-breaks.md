---
description: 默认情况下，当用户搜索广告时间时，TVSDK会强制播放广告时间。 如果距上一个广告时间完成的时间在特定的分钟数内，您可以自定义跳过广告时间的行为。
title: 跳过一段时间的广告时间
...
>[!IMPORTANT]
>
>如果必须完成内部搜寻才能宽恕广告，则播放期间可能会稍有暂停。

要覆盖默认的TVSDK广告时间行为，您可以扩展默认的广告策略选择器。 有四种广告时间策略可用：
...
* 跳过
  ...
  >[!NOTE]
  >
  >当直播点上有广告时，SKIP广告时间策略可能无法正常工作。 例如，对于前置广告，SKIP会导致搜寻到广告时间的结尾，这可能大于实时点。 在这种情况下，TVSDK可能会尝试访问广告中间。
...
* 移除
  ...
  >[!NOTE]
  >
  >此 `REMOVE` 广告时间策略已计划弃用。 Adobe建议您使用 `SKIP` 广告时间策略取代 `REMOVE`.

以下自定义广告策略选择器的示例在用户观看广告时间后的五分钟（墙上时钟时间）内跳过广告。

1. 当用户观看完广告时间时，可节省当前系统时间。
