---
description: 本节介绍配置输入的语法，重点介绍有效和无效的输入选项，并说明如何解释省略的可选字段。
title: RBOP语法
...
本节介绍配置输入的语法，重点介绍有效和无效的输入选项，并说明如何解释省略的可选字段。
...
>[!NOTE]
>
>为了帮助提高语法的可读性，以下属性未反映在语法中，但仍成立：

1. 在对象中定义的对的顺序不是固定的；因此，对的任何排列都是有效的。
...
1. 某些规则允许多个表单。 在这些情况下，不同的形式会由 `'|'` 字符。
   ...
   表示实例 `Foo` 可替换为“A”、“B”或“C”。 这不应与跨多行的表单混淆；这是使较长的表单更易读的功能。
...
此 *示例输出保护配置* 主题给出了一个有效的配置及其语义含义。 中的上一节 *此* 主题介绍了配置的语法规则。 虽然语法有助于确保语法正确，但存在语义不正确的语法合法配置（即，它们不是逻辑配置）。 此部分介绍以下配置 *语法* 合法的，但是 *语义上* 不正确。 请记住，本节中的示例已简化为说明所讨论情景所需的最低结构。
