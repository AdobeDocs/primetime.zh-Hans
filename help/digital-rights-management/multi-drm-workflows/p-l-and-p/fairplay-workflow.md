---
description: DRM工作流包括打包您的内容、为内容提供许可以及从您自己的视频应用程序中回放受保护的内容。 每个DRM解决方案的工作流程通常都相似，但细节中有一些差异。
...
DRM工作流包括打包您的内容、为内容提供许可以及从您自己的视频应用程序中回放受保护的内容。 每个DRM解决方案的工作流程通常都相似，但细节中有一些差异。

此多DRM工作流程将引导您完成使用Apple FairPlay保护的HLS内容的设置、打包、授权和回放。 此工作流程还包括用于实施脱机播放和许可证轮换的可选说明。

## 为FairPlay {#enable-expressplay-service-for-fairplay}启用ExpressPlay服务
...
1. 生成证书签名请求。
   ...
   可以使用[!DNL openssl]生成公钥/私钥对和证书签名请求(CSR)。
   ...
   1. 生成您的CSR。
      ...
      >[!NOTE]
      >
      >此步骤的说明位于&#x200B;*FairPlay流部署包*&#x200B;中，但为方便起见，此处提供。 如果您对此部分流程有问题，请检查&#x200B;*FairPlayCertificateCreation.pdf*（在部署包中）中的说明。
...
1. 通过Apple开发人员门户上传您的CSR。
   ...
   1. 单击页面右上角的&#x200B;**[!UICONTROL +]**&#x200B;按钮以请求新证书。 在&#x200B;**[!UICONTROL Production]**&#x200B;下选择&#x200B;**[!UICONTROL FairPlay Streaming Certificate]**&#x200B;选项。
   ...
   1. ASK中的键完成证书生成，然后单击&#x200B;**[!UICONTROL Continue]**。
   1. 验证是否已保存ASK后，单击&#x200B;**[!UICONTROL Generate]**&#x200B;继续。

      >[!NOTE]
      >
      >保存ASK副本并安全地存储它非常重要。 *如果您的ASK受到损害，您将无法再通过FairPlay流保护您的内容。* 只有一(1)个ASK分配给您的团队。将不再提供该值，您无法在以后检索它。
...
1. 使用您的FairPlay凭据设置您的ExpressPlay帐户。
   ...
   1. 识别公司的FairPlay证书。
      ...
      您的公司名应与Apple颁发的证书关联。
   1. 通过选择展开箭头，然后右键单击您的私钥来展开证书。
   1. 选择&#x200B;**[!UICONTROL Export "Your Company Name"]**&#x200B;并保存[!DNL .p12]文件。

      系统将要求您指定密码以保护此文件。 请记下此密码，因为您需要随凭据包一起发送此密码。
   1. 登录[www.expressplay.com](https://www.expressplay.com)上的帐户。
   1. 单击左上角的&#x200B;**[!UICONTROL DRM SERVICES]**，然后选择&#x200B;**[!UICONTROL FairPlay]**&#x200B;选项卡。
   1. 将您的FairPlay凭据上传到您的ExpressPlay帐户。

      1. 创建一个包含ASK值的文本文件(应该包含32个字符，例如：`1234567890abcdef1234567890abcdef`)，并选择此文件进行上载。
      1. 从步骤4.f中选择PKCS12文件。。
...
### 打包FairPlay的内容{#package-your-content-for-fairplay}

要打包内容，您可以使用Adobe Offline Packager或其他工具，如ExpressPlay的Bento4打包程序。

包装人员准备要回放的视频（例如，将原始文件分片并放入清单中），并使用您选择的DRM解决方案（在本例中为FairPlay）保护视频：
...
1. 打包您的内容。
   ...
   * `drm_sys`  — 要打包的DRM解决方案。在本例中为`FAIRPLAY`。
...
### 设置FairPlay {#setting-policies-for-fairplay}的策略

您可以使用授权服务器为受FairPlay保护的内容设置策略。 您可以设置自己的，或利用Adobe提供的示例授权服务器。

Adobe提供了一个ExpressPlay授权服务器(SEES)示例，用于说明如何执行&#x200B;*基于时间的*&#x200B;和&#x200B;*设备绑定*&#x200B;授权。 此示例授权服务器构建在ExpressPlay服务之上。
...
## FairPlay {#licensing-and-playback-for-fairplay}的许可和播放

许可和播放受FairPlay保护的内容需要在视频清单文件(skd:)中使用的方案与ExpressPlay令牌请求(https:)中使用的方案之间交换URL方案。

有关从iOS TVSDK客户端实现许可和回放的说明，请参见：[在TVSDK应用程序中启用Apple FairPlay](../../../programming/tvsdk-3x-ios-prog/ios-3x-drm-content-security/ios-3x-apple-fairplay-tvsdk.md)。 您还可以选择为FairPlay实施脱机播放和许可证轮换。

## HLS脱机使用FairPlay {#section_047A05D1E3B64883858BC601CFC8F759}

您可能希望使用户能够在无法检索到FairPlay保护的内容时播放其内容，因为播放器与Web隔离（例如在飞机上）。

在开始此任务之前，请下载并阅读Apple文档&#x200B;**&quot;Offline Playback with FairPlay Streaming and HTTP Live Streaming&quot;**。 阅读指南，了解如何下载传输流(TS)区段并将其保存到本地计算机。
...
>[!NOTE]
>
>如果保留的内容密钥已过期，则客户端上的FairPlay流不会开始解密。 但是，如果内容密钥在播放期间过期，它将继续用户体验。
>
...
在M3U8清单中，每个关键标签将应用于以下TS区段，直到下一个关键标签或直到文件结束。
...
* 在许可证轮换期间插入新的FairPlay密钥标签。
   ...
   对于线性内容，请确保在M3U8窗口中保持最新的键标签。 当有两个TS区段有待播放时（大约20秒），iOS将请求下一个M3U8。 如果新的M3U8包含新的密钥标签，则所有密钥请求都将立即发生。 将不再请求以前的现有密钥。 iOS将等待所有关键请求完成，然后播放将开始。
