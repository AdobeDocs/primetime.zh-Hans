...
1. 从Apple获取凭据。
   ...
   在您的请求获得批准后，Apple将向您发送一 *个FairPlay流部署包*。
1. 生成证书签名请求。
   ...
   1. 生成CSR。
      ...
      >[!NOTE]
      >
      >此步骤的说明位于您的FairPlay流 *部署包中*，但为方便起见，此处包含这些说明。 如果您对此过程部分有任何问题，请查看FairPlayCertificateCreation. *pdf(在部署包中* )中的说明。

1. 通过Apple开发人员门户上传您的CSR。
   1. 开发团队的团队代理必须登录 [!DNL developer.apple.com/account]。
   1. 单击 **[!UICONTROL Certificates, Identifiers & Profiles]**，然后选 **[!UICONTROL iOS, tvOS, watchOS]** 择页面左上角的下拉框，然后 **[!UICONTROL Certificates->Production]** 单击页面左侧的。
   1. 单击 **[!UICONTROL +]** 页面右上角的按钮以请求新证书。 选择下 **[!UICONTROL FairPlay Streaming Certificate]** 面的选 **[!UICONTROL Production]**&#x200B;项。
   ...
   1. 记下您的ASK并将其存储在安全位置。
   1. 在ASK中输入完成证书生成的密钥，然后单击 **[!UICONTROL Continue]**。
...
1. 使用您的FairPlay凭据设置您的ExpressPlay帐户。
   ...
   1. 选 **[!UICONTROL Export "Your Company Name"]** 择并保存 [!DNL .p12] 文件。
...
现在，您可以使用ExpressPlay for FairPlay服务创建具有FairPlay内容保护的iOS [!DNL fairplay.cer] 应用程序或HTML5页面以及您的证书。
...
1. 打包您的内容。
   ...
   以下是使用Adobe脱机包装程序的打包示例。 Packager使用配置文件（例如）, [!DNL fairplay.xml]它类似于：
   ...
   * `drm_sys` -要打包的DRM解决方案。 这就 `FAIRPLAY` 是这个。
   ...
   * `content_id` -默认值。
   ...
   如Packager文档 [中所述](https://helpx.adobe.com/content/dam/help/en/primetime/guides/offline_packager_getting_started.pdf#page=7),“作为最佳实践，请创建一个配置文件，其中包含要用于生成输出的常用选项。 然后，通过将特定选项作为命令行参数提供来创建输出。”
...
Adobe提供示例ExpressPlay授权服务器(SEES)，用于显示如何 *执行基于时间**和设备绑定授权* 。 此示例授权服务器构建在ExpressPlay服务之上。
...
>
>有关 [更多详细信息，请参阅使用](https://developer.apple.com/library/content/documentation/AudioVideo/Conceptual/MediaPlaybackGuide/Contents/Resources/en.lproj/HTTPLiveStreaming/HTTPLiveStreaming.html#//apple_ref/doc/uid/TP40016757-CH11-SW3) HTTP实时流化文档。
