...
清单服务器发送的VMAP文件(如 `pttrackingversion=vmap` 果具有以下示例的格式)来自典型的VMAP块。 本发明结构简单，避免了不必要的重复，结构更清晰。 省略号（三个点，以空格分隔）表示某些URL中和某些代码块之间省略的信息。 未缩短的URL显示在多行上，但它们显示在VMAP文件的单行上。
