...
description: 《适用于iOS的TVSDK 1.4发行说明》介绍了TVSDK iOS 1.4的新增或更改内容、已解决的已知问题和设备问题
...
《适用于iOS的TVSDK 1.4发行说明》介绍了TVSDK iOS 1.4的新增或更改内容、已解决的已知问题和设备问题。

## 新增功能 {#new-features}

**版本1.4.45**

* 为了符合Xcode10，TVSDK已从&quot;`libstdc++`从“ ”到“`libc++`“，因此，支持的最低版本为iOS 7。 之前是iOS6。
...
* 在PTAdMetadata接口中添加了布尔类型的enableVodPreroll属性。 属性可用于对VoD流启用前置滚动。 如果enableVodPreroll为NO，则PSDK不播放前置任务。 然而，这不会对中型企业造成影响。 enableVodPreroll的默认值为YES。
...
此版本中未添加新功能。 有关已修复问题的列表，请参阅已解决的问题。
...
* **allowExternalRecording**：引入了一个新的API，以允许安全内容的播放镜像。 Airplay镜像被视为录制，因此allowExternalRecording值必须设置为“True”，以允许进行Airplay镜像，或设置为“False”以停止播放镜像以保护内容。 默认情况下，值为true。
...
* **CRS广告的多CDN支持** TVSDK现在支持CRS广告的多CDN 。 通过提供CRS广告的FTP详细信息，您可以指定CDN位置，而不是Adobe拥有的默认CDN（如Akamai）。

**版本1.4.29**

在PTSDKConfig类中，添加了forceHTTPS API。

PTSDKConfig类提供了一些方法，用于在向Adobe Primetime ad decisioning、DRM和Video Analytics服务器发出的请求上强制SSL。 欲了解更多信息，请参见 `forceHTTPS` 和 `isForcingHTTPS` 此类上的方法。 如果清单通过HTTPS加载，则TVSDK会保留内容使用的HTTPS，并在从该清单加载任何相对URL时遵循此用法。

**注意**：不会修改对第三方域（如广告跟踪像素、内容和广告URL）的请求以及类似请求，内容提供商和广告服务器负责提供通过HTTPS支持的URL。

**版本1.4.18**

Primetime iOS TVSDK现在支持VPAID 2.0 Javascript创意，以实现丰富的交互式流中广告体验。
...
**注意**：Nielsen模块已从TVSDK内部版本中删除，TVSDK将在不久的将来使用新的Nielsen集成模块进行更新。

* **广告回退，在广告选择逻辑中进行Daisy链接(Zendesk #3103)**

对于启用了回退规则的VAST广告（创意），TVSDK会将具有无效MIME类型的广告视为空广告，并尝试在其位置使用回退广告。 您可以在某些方面配置回退行为。
...
在1.4 TVSDK更新中，我们现在还支持进入线性内容的区域封锁或从该区域封锁返回的操作。 TVSDK现在可以并行处理两个清单文件（主文件和备用文件），以监控中断信号，即使备用程序正在代替原始程序显示时也是如此。
...
* **删除/替换C3广告**
   ...
   * 现在，无需额外的准备工作即可将新广告动态插入到从C3窗口出来的视频点播(VOD)资产中。 TVSDK现在提供了一个API，用于删除自定义内容范围并动态插入新广告。 在广播期间播放实时/线性内容，并且在不用适当时间“清理”资产的情况下立即提取内容作为按需内容时，这项强大的新功能也很有用。
...
* DRM支持通过添加forceHTTPS和isForcingHTTPS API来强制HTTPS。
...
* 票证#36294 - iOS TVSDK在Xcode 10中不起作用
   ...
   * 修复了XCode 10上TVSDK的编译问题。 由于XCode 10要求，在TVSDK上为iOS 1.4.45及更高版本构建的应用程序需要最低部署目标为iOS 7.0

* 票证#36321 - PTMediaPlayer和AVPlayer实例之间在“正在播放”状态的可搜索范围中观察到的差异。
...
* 票证#34683 — 广告播放进度时间为负数

   * 引入其他检查来处理广告服务器报告的持续时间与实际广告内容不匹配的情况。

* 票证#34801 — 在暂停状态期间搜索新位置时，currentTime和localTime未更新

   * 现在，如果播放器处于暂停状态，则播放器的当前时间可以设置为零；以前，当前时间仅用于播放状态时设置为零。

* 票证#35037 — 从基于信号的广告插入返回时，播放停止，并且URL无效。

   * 改进了对1.4.42版本中出现的已关闭问题#34385的修复。 添加了isCanceled检查和异常处理代码，使操作队列更加稳健。

**版本1.4.43**

* (ZD#32990) - iOS：在某些提示点上播放内容而不是广告。 &#39;selectedMediaOptionInMediaSelectionGroup&#39; API是AVPlayerItem接口的一部分，现已移至iOS 11中的AVMediaSelection下。 使用此新API解决了此问题。
* (ZD#33683) TVSDK从元数据字符串中删除==后缀。 该问题已在解析逻辑中修复。
...
* (ZD#34684) — 应用广告跳过策略时，前置广告帧会显示几秒钟。 引入了一个新的API enableVodPreroll ，以禁用vod播放中的前置播放。 此API的默认值为“是”。 该API确保跳过主内容中的广告内容拼合。
* (ZD#34765) — 调用stop()后，仍会下载少数Transport Streams区段。 增强了Stop() API以避免下载额外的区段。
* (ZD#34865) - iOS上的直播流前置广告被截断。 与iOS11相关，并添加其他检查以确认流是前置内容还是主内容，从而解决了此问题。
...
* (ZD#33904) - TVSDK未注册AVAudioSessionMediaServicesWereLostNotification和AVAudioSessionMediaServicesWereResetNotification的AVFoundation通知。

  现在可以在播放器应用程序上注册PTMediaServicesWereLostNotification和PTMediaServicesWereResetNotification，以便在媒体服务重置或丢失时获取通知。
...
* (ZD #30371) — 当我们在线性流中插入超过2个广告时，AdBreak开始时间会更改

  修复了尝试在Apple TV上播放内容时导致完全无法播放的错误
* (ZD #32146) — 阻止iOS 11开发测试版时，未收到有关HLS实时内容的PTMediaPlayerStatusError

  使用Charles进行阻止时，未收到有关HLS实时和VOD内容的PTMediaPlayerStatusError（删除连接和403）
* (ZD #29242) — 启用广告后，Airplay视频播放失败

  当启用广告并启用AirPlay开始播放视频时，视频播放从不开始，并且不显示错误
* (ZD#33341) - DRMInterface.h触发Xcode 9中的生成警告
...
* (ZD #31951) — 广告时间为空白屏幕，无广告时间结束。
  ...
  处理了Facebook VPAID广告通常在单一\&amp;lt；AdParameters\&amp;gt；VAST节点中返回多个CDATA块的问题。
* (#33336新西兰元) - [iOS] TVSDK — 尽管Freewheel返回的广告已足够，但广告盒未填充。
...
根据CRS标准化规则在CRS请求中使用创意ID和AdSystem
...
问题已修复。 iOS TVSDK引发异常(AUDNetworkAdInfo：：initWithAdId)并且未处理它。 出现异常是由于广告ID为空。
...
某些情况下无法正常工作的画中画功能已得到修复。

* (ZD# 25246)不正确的广告时间信号

通过调整变体清单中的不连续性标记，解决了此问题。

* (ZD# 26218)尝试在客户端的应用程序框架中包含PSDKLibrary.framework时，应用程序构建过程变得复杂

通过按请求打包PSDKLibrary.framework，此问题得以解决。
...
通过将assetDuration API添加到解决了此问题 `PTVideoAnalyticsTrackingMetadata` 更新实时/线性流的资产持续时间，并提供用于检查实时流的逻辑。
...
* (ZD #24575) — 当enableDebugLog=true时，32位设备上的TVSDK崩溃

修复了在启用日志记录时导致32位设备崩溃的日志格式问题。

**版本1.4.26** (1.4.26.702)适用于iOS 6.0+

* (ZD# 20213) - XCode7的TVSDK FW需要动态/模块化
...
在删除旧项目后但在将新项目添加到AVQueuePlayer之前添加等待时间解决了此问题。 如果没有等待时间，则通知将发送到不正确的项目。
...
通过改进发生来自AVFoundation的致命错误时的错误通知并允许应用程序处理错误后的重新启动，此问题已得到解决。

* (ZD #20580) - PTSpplicerManager中的崩溃
...
在为Apple TV 4设备打开播放功能后，通过添加实时视频暂停的解决方法，解决了此问题。 问题似乎是AppleTV 4问题。
...
PTNetworkConfiguration类中添加了API，以将Cookie作为某些Akamai标记化的流区段的URL参数附加到这些流中。
...
已修复重定向问题，因为重定向URL在可用于分析清单之前已被cleanConnectionData清除。
...
通过将DRM m3u8播放列表重写优化为先前已重写的播放列表的缓存位，该问题得以解决。 当您播放实时m3u8流时，这一点最为相关，每次下载区段后，都会下载这些流的m3u8。

* (ZD#18956) — 在iOS Demo Player中设置断点时，player.drmManager为零

通过更新PTMediaPlayer.drmManager API实现以从DRM框架获取DRManager，此问题已得到解决。
...
解决了当AuditudeURL的开头包含空格时，跟踪SDK无法发送Ping的问题。
...
* Zendesk #18166 - TVSDK 1.4.15在使用dSYM文件选项编译DWARF时会发出数百条警告
...
恢复OKHTTP对CRS审核的依赖关系，因为TVSDK现在直接使用httpurlconnection而不是curl。 通过先清除例外，然后再进行另一个JNI调用，该问题得以解决。
...
当TVSDK库使用进行YUV ->RGB颜色转换的霓虹代码时，TVSDK在Android M OS上崩溃。 通过使用非霓虹灯版本的代码更新导致此问题的函数，已解决此问题。

* Zendesk #18072 - Android M — 应用程序崩溃

当在检查配置文件和级别是否受支持时调用MediaCodecList和MediaCodecInfo API时，会发生此崩溃。 Adobe正在寻求Google对额外洞察的支持。 该问题通过提供临时解决方法来解决，方法是提前加载所有编解码器信息，以避免仅在需要编解码器信息时调用这些API。
...
**注意**：Nielsen模块已从TVSDK内部版本中删除，但是TVSDK将在不久的将来通过新的Nielsen集成模块进行更新。

* (ZD #2228) — 获取在MediaPlayerNotification中不可用的清单时返回的错误

添加了元数据，以便在出现通知M3U8_PARSER_ERROR时公开内容。
...
支持固定空广告时间跟踪URL，TVSDK现在将验证空广告时间的广告时间开始和结束Ping
...
* (ZD #2141)适用于TreeHouse应用程序的Analytics实施添加了AdobeAnalyticsPlugin.a库来构建包。
* 视频心率库更新至1.4.1.2
...
* 在iOS TVSDK中，所有广告都会合并到内容清单中。 广告行为是通过基于内容和广告区段的持续时间进行搜索来实现的。 因此，如果区段持续时间不准确，则搜寻可能不会始终在广告时间的开始或结束的确切帧结束。 即使帧具有持续时间，平台本身也会对搜索施加容忍，并且可能会显示一些帧或广告或内容。 这是平台的一个限制，而且广告插入在iOS上与TVSDK配合使用的方式也存在限制。
* 在这种情况下，跳过的决定发生在搜寻事件中。 但是，由于清单中的广告区段持续时间无法准确表示广告的实际持续时间，因此搜寻的帧准确性不高。 因此，在应用广告策略时，您会看到广告的几个帧。
...
* 将最小目标设置为iOS7（或更高版本）时，drmNativeInterface.framework无法正确链接。\
  解决方法：明确指定 `libstdc++6`.  dylib库，如下所示：转到Target ->生成阶段 — >将二进制文件与库关联并添加 `libstdc++.6.dylib`.

* 未插入用于替换API的后置广告。
* 在广告时间中搜寻（不退出）会发出重复的广告开始广告时间通知
* 设置currentTimeUpdateInterval没有任何效果。\
  注意：在某些iOS版本中，操作系统不会自动加载PSDKLibrary.framework中的资源。 请务必手动将PSDKResources.bundle复制到应用程序的捆绑包资源：转到“构建阶段”并复制捆绑包资源。
