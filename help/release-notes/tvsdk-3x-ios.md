---
title: TVSDK 3.13 for iOS发行说明
description: TVSDK 3.13 for iOS发行说明描述了TVSDK iOS 3.13中的新增或更改功能、已解决和已知问题以及设备问题。
...
有关对客户报告的问题的修复，请参阅[已解决的问题](#resolved-issues)。 有关限制，请参阅[已知问题和限制](#known-issues-and-limitations)。

### 早期版本{#whats-new-previous}中的新增功能和修复
...
为`isFallbackOnInvalidCreativeEnabled`和方法`customParams`导致应用程序崩溃的客户问题提供了修复。

**iOS TVSDK 3.10**

* 修复了当网络不可用时TVSDK播放器不触发`PTMediaPlayerStatusError`通知的问题。
...
iOS 13规范和iOS 13 UIebView API弃用。

**iOS TVSDK 3.7**

在同时发出大量广告分辨率请求时停止播放的场景的修补程序。

**iOS TVSDK 3.6**

**类的vastXML属性中的修复`PTNetworkAdInfo`**

vastXML属性设置不正确，返回的值为nil。
...
要启用此功能，我们需要设置在PTMediaPlayer类中添加的新API `audioPlaybackInBackground`。 启用此API后，您的应用程序即可播放背景音频。
...
* TVSDK 3.4中，用户现在可以为整体广告分辨率和清单下载设置超时值。 如果在给定超时内，某些广告    已解决，TVSDK将播放其余广告。

* PTAd元数据：adRequestTimeout API已弃用，将被删除。 默认值已设置为35秒。

* PTAdMetadataClass中引入了两个新的备用API:adResolutionTimeout — 针对广告总体分辨率调用的超时                adManifestTimeout — 广告清单下载超时。

**收入优化**

启用TVSDK以识别与广告插入工作流相关的问题区域，以向分析最终选择点报告。

**版本3.3**

TVSDK 3.3现在符合iOS 11 SDK。 所有已弃用的API都已替换为合适的替代项。

**版本3.2**

**其他日志记录支持（第2阶段）**

添加了对错误通知的支持，以防发生以下情况：
...
* **其他日志记录**
支持在广告播放失败时添加对描述性通知的支持。

* **增加了Fairplay加密CMAF流支**
持现在支持Fairplay加密CMAF流和AVC编解码器回放。
...
在应用程序级接口上添加了布尔类型的`enableDelayAdLoading`属性以启用JIT。 如果`enableDelayAdLoading`为NO，则它将`setadMetadata.delayAdLoading`变为True（PTAdMetadata接口的属性）。
...
此版本中未添加任何新功能。 有关已修复的问题的列表，请参阅[已解决的问题](#resolved-issues)。
...
* **allowExternalRecording**:引入了新的API以允许对安全内容进行空播镜像。Airplay镜像视为录制，因此必须将`allowExternalRecording`值设置为`True`，以允许Airplay镜像，或将其设置为`False`以停止安全内容的Airplay镜像。 默认情况下，`value`为true。
...
在iOS TVSDK中集成和验证VHL 2.0:通过降低API的复杂性，减少`VideoHeartbeatsLibrary`实现中的障碍。
...
在`PTSDKConfig`类中，已添加forceHTTPS API。

`PTSDKConfig`类提供了对向Adobe Primetime广告决策、DRM和视频分析服务器发出的请求强制使用SSL的方法。 有关详细信息，请参阅该类的`forceHTTPS`和`isForcingHTTPS`方法。 如果清单是通过HTTPS加载的，则TVSDK保留HTTPS的内容使用，并在从该清单加载任何相对URL时遵守此用法。

>[!NOTE]
>
>对第三方域（如广告跟踪像素、内容和广告URL）的请求以及类似请求不会进行修改，内容提供者和广告服务器有责任提供通过HTTPS支持的URL。
...
对于启用回退规则的VAST广告（创意），TVSDK将MIME类型无效的广告视为空广告并尝试使用回退广告替代广告。 您可以配置回退行为的某些方面。 有关详细信息，请参阅VAST和VMAP广告的广告回退。
...
* **视频心率库(VHL)更新至版本1.4.1.1**
   ...
   * 已通过删除`trackAdBreakStart`和`trackAdBreakComplete`方法优化了广告跟踪。 从`trackAdStart`和`trackAdComplete`方法调用推断出广告中断。
   * 跟踪广告时不再需要`playhead`属性。
   * 增加了对Marketing Cloud访客ID的支持。

* **Nielsen SDK集成**

TVSDK现在支持将mTVR和MDPR ID3信标发送到Nielsen SDK，而无需任何自定义集成。 要开始，请下载3.1.2.19 Nielsen iOS App SDK，然后按照iOS Programers Guide中此处提供的说明操作。
...
* (ZD 42085) — 与在CMAF流上播放有关的问题。

* (ZD-43215) — 在广告进行中取消播放器时崩溃。

* (ZD 43210) — 启用WebVTT子标题时，iOS HLS播放将冻结。

**iOS TVSDK 3.12**

* 在使用适用于iOS 3.10的TVSDK时，在播放15分钟后，实时流将失败。
...
* (ZD#41289) — 使用方法`customParams`观察到`NSInvalidArgumentException`导致应用程序崩溃。
...
(ZD#40272)- iOS TVSDK无法播放VTT字幕，101001错误并导致应用程序冻结。

**iOS TVSDK 3.8**

* (ZD#40087)- iOS崩溃，因为播放器错误导致过期的VOD内容。

* (ZD#40083)- Pre-Roll广告不播放带有`OpportunityGenerator`的实时流，播放器会给出错误。

* (ZD#39828)- `CurrentItem`属性缺少可空注释，当通知中包含的播放器状态为`PTMediaPlayerStatusStopped`时，会导致播放器崩溃。

**iOS TVSDK 3.7**

(ZD#38961) — 当将多个内容配置为在PiP中播放时，当一个内容完成播放后，内容无法在“画中画(PiP)”窗口中播放。
...
(ZD#37820) — 添加了自定义头HS-Id、HS-SSAI-TAG的允许列表。
...
* **Ticket#37080 — 已看到** “清单”呼叫的重复请求。修复了在播放期间对清单URL发出的重复请求。 现在，TVSDK对每个清单发出一个呼叫。

* **Ticket#37** - CRS标准化规则失败，且EQ匹配类型修复了播放器在遇到具有“eq”匹配类型主机名的上次标准化规则集时崩溃的情况。

**版本3.1**

**票证号36313**  — 线性广告中断期间的间歇性不可预知结果在实时流中线性广告中断期间固定的间歇性播放。

**版本3.0.1**

**Ticket36948** - CRS — 在iOS 12上不一致的资产选择顺序为CRS选择的资产并不总是在VAST或VMAP响应中返回的最高质量变体。

**版本3.0**

* **Ticket35311**  — 在电话呼叫中断期间，播放器状态未变为“暂停”添加的中断处理程序可阻止播放器中断。中断时，播放器状态变为“暂停”，然后单击播放按钮后继续播放。

* **Ticket36685 — 实时资源**  — 时间与播放器时间进度和SCTE标记时间不匹配对于处于实时点之前的SCTE标记，会计算正确时间。

* **Ticket36492**  — 在 `currentTime` 暂 `localTime` 停状态期间搜索到新位置时未更新Ticket36492 — 在播放器处于暂停状态时，播放器的当前时间现在可以设置为零；之前，仅在播放状态中将当前时间设置为零。

**版本1.4.45**

* **Ticket36294** - iOS TVSDK在Xcode 10上无法工作修复了XCode 10上TVSDK的编译问题。由于XCode 10要求，从iOS 1.4.45版的TVSDK上构建的应用程序要求最低部署目标，因为iOS 7.0

* **Ticket36321**  — 在“播放”状态中实例 `PTMediaPlayer` 与实 `AVPlayer` 例之间可搜索范围中观察到的差异。

* **Ticket36493-**  iOS  `libstdc++` 12上的支持修复了iOS 12上TVSDK的编译问题。从iOS 1.4.45版到TVSDK上构建的应用程序需要最低的部署目标，如iOS 7.0

**版本1.4.44**

* **Ticket34683**  — 广告播放进度时间为负

当广告服务器报告的持续时间与实际广告内容不匹配时，会引入其他检查来处理这种情况。

* **Ticket34801**  — 当在暂停状态期间搜索到新位置时，当播放器处于暂停状态时， Player的当前时间现在可以设置为零；之前，仅在播放状态中将当前时间设置为零。

* **Ticket35037**  — 从基于信号的广告插入返回时，播放停止，URL不正确。改进了针对1.4.42版中#34385问题提供的修复。 添加了isCancelled检查和异常处理代码，使操作队列更加健壮。

**版本1.4.43**

* (ZD#32990)- iOS:在某些提示点上播放内容，而不是投放广告。 `selectedMediaOptionInMediaSelectionGroup` 作为AVPlayerItem接口一部分的API现在已移到iOS 11中的AVMediaSelection下。使用此新API解决了此问题。
...
* (ZD#34385) — 从基于信号的广告插入返回时，播放会停止，并且URL不正确。
   ...
   增加`CustomAVAssetLoaderOperations`的最大并发计数，以便清单读取继续执行。
...
* (ZD#33904)- TVSDK未针对AVFoundation通知`AVAudioSessionMediaServicesWereLostNotification`和`AVAudioSessionMediaServicesWereResetNotification`注册。

   `PTMediaServicesWereLostNotification` 并且 `PTMediaServicesWereResetNotification` 现在可以在播放器应用程序上注册，以在媒体服务重置或丢失时获取通知。
...
* (ZD #32146) — 阻止iOS 11开发测试版上的HLS Live内容未收到`PTMediaPlayerStatusError`

   使用Charles（Drop connection和403）阻止时，未收到HLS Live和VOD内容的`PTMediaPlayerStatusError`。

* (ZD #29242)- Airplay视频播放在启用广告后失败。

   启用广告并启用AirPlay后，开始播放视频时，视频播放不会开始，不会显示错误。

* (ZD#33341)- `DRMInterface.h`触发Xcode 9中的生成警告。

   修复了`DRMInterface.h`中缺少参数列表中的“void”一词的两个块原型。
...
* (ZD #32465) — 播放器无法处理合并的播放列表。
   ...
   调用`finishLoadingWithError`(具有：错误)，以使AV基础尝试替代流/触发故障转移。
...
* (ZD #33336)- iOS TVSDK — 尽管Freewheel返回了足够多的广告，广告窗格仍未填充。
...
* (ZD #29176)- `PTAdPolicyDeligate` `satAdBreakAsWatched:position`崩溃
...
问题已修复。 iOS TVSDK正在提出`exception(AUDNetworkAdInfo::initWithAdId)`，但未处理它。 此异常是由于空广告ID。
...
* (ZD# 27460)Midroll第一个广告呼叫 — POST到`cdn.auditude.com`返回403。
...
通过将assetDuration API添加到PTVideoAnalyticsTrackingMetadata，可更新实时/线性流的资产持续时间并提供检查实时流的逻辑，从而解决了此问题。
...
* (Zendesk #18166)- TVSDK 1.4.15在使用dSYM文件选项的DWARF进行编译时会发出数百个警告
...
* (Zendesk #17919)- Android — 内容搜索导致心率错误

问题是当在章节中存在搜索时，解决错误状态中的心跳

* (Zendesk #18053) — 使用TVSDK的应用程序在Marshmallow上崩溃

当TVSDK库使用执行YUV `->` RGB颜色转换的neon代码时，TVSDK在Android M OS上崩溃。 通过使用非neon版本的`code`更新导致此问题的函数，解决了此问题。
...
* (Zendesk #18074) — 对于Nexus和Android 6.0，阿拉伯语字幕不起作用
...
* (ZD #2141)TreeHouse应用程序的分析实施，添加了`AdobeAnalyticsPlugin.a`库以构建包。
* 视频心率库更新到1.4.1.2
...
* 通过添加`forceHTTPS`和`isForcingHTTPS` API来强制使用HTTPS的DRM支持。
...
* 在这种情况下，在搜索事件上会做出跳过的决定。 但是，由于清单中的广告段持续时间不能准确地表示广告的实际持续时间，因此搜索不准确。 因此，在应用广告策略时，您会看到几个广告帧。
...
* 启用“仅在时间内”广告解决时，详细错误通知不可用。
* 错误通知按广告解决时间记录，而非按广告序列记录。
* HEVC支持在此版本中有以下限制
   * 不支持DRM
   * CC(CEA 608/708)支持不可用，因为CMAF中不支持它。
   * 尚未提供4K支持。
   * ID3标记不支持，因为CMAF中不支持它。
   * 未验证未混合的实时HEVC流。
   * 未验证HEVC广告支持。
* 启用JIT并将容差设置为10秒后，在出现VMAP->VAST重定向广告时，将不会看到第一个中间广告中断的VAST调用。
* 为了使广告分辨时间正常工作，每次在实时流播放期间更新播放列表时，播放器都需要20秒内拼接的播放列表。 如果在所述间隔内未收到拼接播放列表，则引发内部错误并且播放器停止。
