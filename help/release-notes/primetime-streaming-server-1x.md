...
description: Primetime Streaming Server 1.3和1.4版中的新增功能。
...
Primetime Streaming Server 1.3和1.4版中的新增功能。
...
* PTPUB- 645 DPISimple Mode强制执行，即使当命令行提示和流内提示均以脱机打包程序配置提供时，指定DPIScte35模式也是如此
...
1. 使用DRM在原点生成MP4 JIT m3u8的性能修复，包括密钥轮替
...
* 3759167 — 由于打包时时间戳异常，并非所有SCTE35提示都可将其添加到输出清单中。 对SCTE35消息中SpliceInfoSection的TimeSignal中的SpliceTime应用pts_adjustment。

### 1.3.1版中的已知问题 {#known-issues-in-release}

* 3717039 — 当将打包器配置为生成DPI简单模式提示时，它实际上应该查找特定的信号类型（例如接合插入或放置机会），并且仅将这些信号转换为简单模式提示。 它应忽略其它类型的信号，如程序启动、网络启动等。

* 3718598 — 当源服务器配置为在启用HSM访问的情况下提供受保护的内容时，后端LunaSA客户端会与HSM模块进行频繁通信

## Primetime Streaming Server 1.3的新增功能（4月版） {#what-s-new-in-primetime-streaming-server-april-release}

Primetime 1.3版本围绕流内容提供了多项新功能，提高了可用性和安全性。

**Primetime Streaming Server作为统一形式的Live Packager和源服务器**

Primetime Live Packager和Primetime Origin结合使用，作为单个组件使用。 此组件可用作打包程序或用作源，或者使用组合的功能来打包和托管实时流。

这为这些服务器提供了统一的文件接口，使它们易于在单个计算机上运行。 它仍然提供了灵活性，可以配置为单独的Packager或Origin。

**Beta MPEG — 短划线支持**

Primetime Streaming Server支持MPEG-DASH打包以用于实时和VOD工作流。 Live Packager组件将摄取RTMP或MPEG-2-TS流转换为短划线格式。 原始组件接受DASH流。

对于VOD工作流，脱机打包程序组件将MP4和TS资源转换为MPEG-DASH ISOBFF格式。

**实时到VOD转换**

新组件记录服务器现已可用，该服务器支持捕获实时流并存档以进行VOD播放。 它支持创建完整的事件重播以及部分事件的剪辑/高亮。 可以将其配置为在实时内容中记录纯音频流、删除广告或幻灯片。 Recording Server可与Primetime Streaming Server以及第三方源配合使用。
...
usermgmt.jar现在随Primetime Live Packager提供，用于在向Primetime Live Packager发送RTMP流时，使用可信凭据配置访问

现在，可以将编码器配置为在向Live Packager发送流时使用用户名/密码。

**用于创建HDS和HLS顶级清单的PlaylistCreator工具**

Primetime Offline Packager现在提供聪明的实用程序PlaylistCreator.jar，可以轻松创建HDS和HLS资产的顶级清单文件。

**附加安全功能，可合并硬件安全模块**

Primetime Offline Packager现在支持从Hardware Security Module访问Packager凭据证书和通用密钥。

Hardware Security模块为这些机密资产提供额外的保护。

**改进了VOD打包性能**

已引入一些性能增强功能，以缩短Primetime Offline Packager中夹层资产的打包时间

**改进了JIT MP4打包性能**

Primetime Origin的JIT打包功能中引入了几项性能增强，以处理用户对大型VOD资产库的请求。
...
2. 解压缩Adobe Primetime-Streaming Server 1.4存档文件， `Primetime- StreamingServer-1-4-0-b206-12042014.zip` 到你的光盘。
...
**将Primetime流服务器配置为实时包管理器或HTTP源服务器**
...
此部分适用于以下情况：未使用Primetime流服务器，而是部署Primetime Live Packager和/或Primetime源服务器
...
上述最低系统要求对于Origin Server和Live Packager都成立。
...
1. 解压缩Adobe Primetime - HTTP Origin Server 1.4存档文件， `Primetime-HttpOrigin-1-4-0-b206-12042014.zip`，放入您的磁盘。
...
要启动HTTP源服务器，请从源服务器的根目录中的命令行执行以下命令：\
`$./origin_start.sh`
...
**注释**：当打包程序启动时，它会尝试从临时目录中的片段目标初始化引导信息。 如果在片段目标上找到引导程序信息，则表示打包程序已重新启动。 如果重新启动，打包程序将等待下一个片段边界，然后开始打包。 打包程序在引导中插入一个间隙条目以指示缺少片段。
...
有关此处提供的配置详细信息，请参阅Primetime HTTP Origin Server入门文档。

**卸载Live Packager**

要卸载打包程序，请停止该打包程序，并从Primetime目录中删除该打包程序目录。
...
1. 提取Adobe Primetime - Offline Packager 1.4存档文件， `Primetime- OfflinePackager-1-4-0-b206-12042014.zip`，放入您的磁盘。

有关可用的配置详细信息，请参阅Primetime Offline Packager入门文档 [此处](https://help.adobe.com/en_US/primetime/api/packagers/offline/index.html).
