...
* CRS: 添加了阻止来自特定广告系统、广告ID、创意ID、创意URL和／或创意格式的列表／允许列表CRS请求的功能

   >注释
   >
   >当找到具有同一广告的动态值（例如，URL中的动态参数）的广告提供者时，Adobe将添加块列表规则。 在动态组件解析后，将禁用此类块列表规则，无论是由提供程序解析还是通过标准化规则解析。

   * 如果要添加阻止列表或允许区域列表规则，请与技术客户经理联系以获得帮助。
