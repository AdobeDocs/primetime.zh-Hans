---
title: 限制属于不同所有者的多个应用程序的并发使用
description: 限制属于不同所有者的多个应用程序的并发使用
...
# 限制属于不同所有者的多个应用程序的并发使用 {#restr-concurr-usage}
...
要与CM集成，必须创建Zendesk票证(https://adobeprimetime.zendesk.com)并指定以下信息：

1. 公司名称
1. 要与CM集成的应用程序。 对于每个应用程序，您需要提供：
   - 应用程序名称
   - 应用平台
1. 请求集成的第三方（如果适用）
